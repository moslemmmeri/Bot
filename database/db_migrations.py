...
CURRENT_VERSION = 4  # نسخه‌ی فعلی دیتابیس (افزایش به ۴ برای مدیریت ستون‌ها)
...
def ensure_migrations_table():
    ...
    این جدول برای ثبت تاریخچه مهاجرت‌ها استفاده می‌شود.
    """
...
def get_current_version():
    """
    دریافت نسخه‌ی فعلی دیتابیس از جدول migrations.
    اگر جدول خالی باشد، ۰ برمی‌گرداند.
    """
...
def record_migration(version, description, checksum=None):
    """
...
MIGRATIONS = {
    ...
    2: {
        ...
        "down": """
            ...
            -- در صورت نیاز، جدول را بازسازی کنید.
    ...
    3: {
        ...
        "up": """
            -- اطمینان از وجود ستون sort_order در جداول اصلی
    ...
    4: {
        ...
        "up": """
            ...
            -- اضافه کردن ستون columns به جدول buttons (برای تنظیمات اختصاصی هر دکمه)
            ALTER TABLE buttons ADD COLUMN columns INTEGER DEFAULT NULL;
            
            -- درج مقدار پیش‌فرض در جدول settings (اگر وجود نداشته باشد)
            INSERT OR IGNORE INTO settings (key, value, description) 
            VALUES ('default_menu_columns', '2', 'تعداد ستون‌های پیش‌فرض منو (۱ تا ۸)');
        """,
        "down": """
            -- SQLite از DROP COLUMN پشتیبانی نمی‌کند، بنابراین این مهاجرت قابل بازگشت نیست.
            -- در صورت نیاز، جداول را بازسازی کنید.
    ...
    5: {
        ...
        "up": """
            -- اضافه کردن ستون price_type به جدول buttons
            ALTER TABLE buttons ADD COLUMN price_type TEXT DEFAULT 'fixed';
            
            -- اضافه کردن ستون min_price به جدول buttons
            ALTER TABLE buttons ADD COLUMN min_price INTEGER;
            
            -- اضافه کردن ستون max_price به جدول buttons
    ...
    6: {
        ...
        "up": """
            -- اضافه کردن ستون is_blocked به جدول users
            ALTER TABLE users ADD COLUMN is_blocked INTEGER DEFAULT 0;
            
            -- اضافه کردن ستون block_reason به جدول users
            ALTER TABLE users ADD COLUMN block_reason TEXT;
            
            -- اضافه کردن ستون blocked_at به جدول users
            ALTER TABLE users ADD COLUMN blocked_at TEXT;
            
            -- اضافه کردن ستون‌های role و status و language و timezone و extra_data
...
def apply_migration(version):
    ...
    try:
        with get_db_connection() as conn:
            ...
            # SQLite از اجرای چندین دستور در یک execute پشتیبانی نمی‌کند،
            # بنابراین دستورات را جدا می‌کنیم
...
def get_migration_sql(version, direction="up"):
    """
...
# ==================== تابع یکپارچه برای استفاده در init_db ====================
